...
pub fn generate_cdxgen_python_dependency_tree(
    project_manipulator: &dyn ProjectManipulator,
) -> Result<DependencyTreeNode> {
    project_manipulator.run_shell("cdxgen -t python -o bom.source-wand.json --output-format json --feature-flags safe-pip-install".to_string())?;
    ...
    let bom: Bom = serde_json::from_str(&bom_raw)
        .map_err(|e| Error::msg(format!("Failed to parse BOM JSON: {}\n\nHere is the BOM in question:\n{}", e, bom_raw)))?;
